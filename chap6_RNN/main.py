        ...
        for line in f.readlines():
            try:
                ...
                if '_' in content or '(' in content or '（' in content or '《' in content or '[' in content or \
                    ...
                    continue
                if len(content) < 5 or len(content) > 80: # 内容太短或者太长的不要
                    continue
    ...
    for epoch in range(5):
        ...
        for batch in range(n_chunk):
            ...
            for index in range(BATCH_SIZE):
                ...
                x = Variable(torch.from_numpy(np.expand_dims(x,axis=1))).to('mps')
                y = Variable(torch.from_numpy(y )).to('mps')
                pre = rnn_model(x)
                loss += loss_fun(pre , y)
                if index == 0:
